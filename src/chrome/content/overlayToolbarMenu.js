...
var gmanager_ToolbarMenu = new function()
{
  ...
  gmToolbarMenu.buildMenu = function(aPopup)
  {
    ...
    composeMenupopup.addEventListener("popupshowing", "event.stopPropagation(); return gmanager_ToolbarMenu.buildComposeMenu(this);");
    ...
    if (accounts.length > 0)
    {
      ...
      menuitem.addEventListener("command", "gmanager_Accounts.loginAllAccounts();");
      ...
      menuitem.addEventListener("command", "gmanager_Accounts.logoutAllAccounts();");
      menuitem.setAttribute("disabled", (numLoggedIn === 0));
      aPopup.appendChild(menuitem);
      
      // Check All Accounts
      menuitem = gmToolbarMenu._createMenuitem("check-all-accounts");
      menuitem.addEventListener("command", "gmanager_Accounts.checkAllAccounts();");
      menuitem.setAttribute("disabled", (numLoggedIn === 0));
      aPopup.appendChild(menuitem);
      ...
      if (gmanager_Toolbars.isToolbarItem(toolbarItem))
      {
        ...
        if (displayAccount)
        {
          if (displayAccount.loggedIn)
          {
            ...
            menuitem.addEventListener("command", "gmanager_Accounts.logoutAccount('" + displayAccount.email + "');");
            aPopup.appendChild(menuitem);
          }
          else
          {
            ...
            menuitem.addEventListener("command", "gmanager_Accounts.loginAccount('" + displayAccount.email + "');");
            aPopup.appendChild(menuitem);
          }
          
          // Check Selected Account
          menuitem = gmToolbarMenu._createMenuitem("check-selected-account");
          menuitem.addEventListener("command", "gmanager_Accounts.checkAccount('" + displayAccount.email + "');");
          ...
          menuitem.addEventListener("command", "gmanager_Alerts.display('" + displayAccount.email + "');");
        ...
        accounts.forEach(function(account, index, array) {
          ...
          menuitem.addEventListener("command", "gmanager_ToolbarMenu.switchAccount('" + account.email + "');");
    ...
    else
    {
      ...
      menuitem.addEventListener("command", "gmanager_Utils.showLogin();");
      aPopup.appendChild(menuitem);
    ...
    menuitem.addEventListener("command", "gmanager_Utils.loadSimpleURI(gmanager_Utils.WEBSITE);");
    aPopup.appendChild(menuitem);
    
    // Options...
    menuitem = gmToolbarMenu._createMenuitem("options");
    menuitem.setAttribute("default", "true");
    menuitem.addEventListener("command", "window.openDialog('chrome://gmanager/content/options/options.xul', 'options', 'centerscreen,chrome,modal,resizable');");
    aPopup.appendChild(menuitem);
  ...
  gmToolbarMenu.buildComposeMenu = function(aPopup)
  {
    ...
    menuitem.addEventListener("command", "gmanager_ToolbarMenu.composeAccount(null);");
    aPopup.appendChild(menuitem);
    ...
    if (accounts.length > 0)
    {
      ...
      accounts.forEach(function(account, index, array) {
        ...
        menuitem.addEventListener("command", "gmanager_ToolbarMenu.composeAccount('" + account.email + "');");
        aPopup.appendChild(menuitem);
      });
    }
